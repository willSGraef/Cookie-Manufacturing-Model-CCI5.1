...
while not shutdown:
    ...
    fan_1 = redis_client.get("fan_1")

    # Compare redis value to OpenPLC value and update if necessary
    if fan_1.get_value() != modbus_client.read_signal(fan_1):
        fan_1.set_value(modbus_client.read_signal(fan_1))
        redis_client.set_value("fan_1", fan_1.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(fan_1)
