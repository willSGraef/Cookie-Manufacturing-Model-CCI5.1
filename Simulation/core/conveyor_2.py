...
while not shutdown:
    ...
    conveyor_2 = redis_client.get("conveyor_2")
    conveying_2 = redis_client.get("conveying_2")

    # Compare redis value to OpenPLC value and update if necessary
    if conveyor_2.get_value() != modbus_client.read_signal(conveyor_2):
        conveyor_2.set_value(modbus_client.read_signal(conveyor_2))
        redis_client.set_value("conveyor_2", conveyor_2.get_value())

    if conveying_2.get_value() != modbus_client.read_signal(conveying_2):
        conveying_2.set_value(modbus_client.read_signal(conveying_2))
        redis_client.set_value("conveying_2", conveying_2.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(conveyor_2)
    modbus_client.write_signal(conveying_2)
