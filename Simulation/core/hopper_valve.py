...
while not shutdown:
    ...
    rv_3 = redis_client.get("rv_3")

    # Compare redis value to OpenPLC value and update if necessary
    if rv_3.get_value() != modbus_client.read_signal(rv_3):
        rv_3.set_value(modbus_client.read_signal(rv_3))
        redis_client.set_value("rv_3", rv_3.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(rv_3)
