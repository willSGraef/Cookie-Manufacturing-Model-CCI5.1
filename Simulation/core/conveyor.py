...
while not shutdown:
    ...
    conveyor = redis_client.get("conveyor")
    conveyor_fpm = redis_client.get("conveyor_fpm")

    # Compare redis value to OpenPLC value and update if necessary
    if conveyor.get_value() != modbus_client.read_signal(conveyor):
        conveyor.set_value(modbus_client.read_signal(conveyor))
        redis_client.set_value("conveyor", conveyor.get_value())

    if conveyor_fpm.get_value() != modbus_client.read_signal(conveyor_fpm):
        conveyor_fpm.set_value(modbus_client.read_signal(conveyor_fpm))
        redis_client.set_value("conveyor_fpm", conveyor_fpm.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(conveyor)
    modbus_client.write_signal(conveyor_fpm)
