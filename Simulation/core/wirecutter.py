...
while not shutdown:
    ...
    wirecutter = redis_client.get("wirecutter")
    wirecut_cpm = redis_client.get("wirecut_cpm")

    # Compare redis value to OpenPLC value and update if necessary
    if wirecutter.get_value() != modbus_client.read_signal(wirecutter):
        wirecutter.set_value(modbus_client.read_signal(wirecutter))
        redis_client.set_value("wirecutter", wirecutter.get_value())

    if wirecut_cpm.get_value() != modbus_client.read_signal(wirecut_cpm):
        wirecut_cpm.set_value(modbus_client.read_signal(wirecut_cpm))
        redis_client.set_value("wirecut_cpm", wirecut_cpm.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(wirecutter)
    modbus_client.write_signal(wirecut_cpm)
