...
while not shutdown:
    ...
    lcm = redis_client.get("lcm")
    mixer = redis_client.get("mixer")
    mixer_rpm = redis_client.get("mixer_rpm")

    # Compare redis value to OpenPLC value and update if necessary
    if mixer.get_value() != modbus_client.read_signal(mixer):
        mixer.set_value(modbus_client.read_signal(mixer))
        redis_client.set_value("mixer", mixer.get_value())

    # Push mixer weight to OpenPLC
    modbus_client.write_signal(lcm)
    modbus_client.write_signal(mixer)
    modbus_client.write_signal(mixer_rpm)
