...
while not shutdown:
    ...
    conveyor_4 = redis_client.get("conveyor_4")
    conveying_4 = redis_client.get("conveying_4")

    # Compare redis value to OpenPLC value and update if necessary
    if conveyor_4.get_value() != modbus_client.read_signal(conveyor_4):
        conveyor_4.set_value(modbus_client.read_signal(conveyor_4))
        redis_client.set_value("conveyor_4", conveyor_4.get_value())

    if conveying_4.get_value() != modbus_client.read_signal(conveying_4):
        conveying_4.set_value(modbus_client.read_signal(conveying_4))
        redis_client.set_value("conveying_4", conveying_4.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(conveyor_4)
    modbus_client.write_signal(conveying_4)
