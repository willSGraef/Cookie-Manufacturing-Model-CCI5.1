...
while not shutdown:
    ...
    conveyor_1 = redis_client.get("conveyor_1")
    conveying_1 = redis_client.get("conveying_1")

    # Compare redis value to OpenPLC value and update if necessary
    if conveyor_1.get_value() != modbus_client.read_signal(conveyor_1):
        conveyor_1.set_value(modbus_client.read_signal(conveyor_1))
        redis_client.set_value("conveyor_1", conveyor_1.get_value())

    if conveying_1.get_value() != modbus_client.read_signal(conveying_1):
        conveying_1.set_value(modbus_client.read_signal(conveying_1))
        redis_client.set_value("conveying_1", conveying_1.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(conveyor_1)
    modbus_client.write_signal(conveying_1)
