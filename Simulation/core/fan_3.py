...
while not shutdown:
    ...
    fan_3 = redis_client.get("fan_3")

    # Compare redis value to OpenPLC value and update if necessary
    if fan_3.get_value() != modbus_client.read_signal(fan_3):
        fan_3.set_value(modbus_client.read_signal(fan_3))
        redis_client.set_value("fan_3", fan_3.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(fan_3)
