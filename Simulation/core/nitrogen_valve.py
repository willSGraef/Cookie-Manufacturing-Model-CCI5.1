...
while not shutdown:
    ...
    gv_1 = redis_client.get("gv_1")

    # Compare redis value to OpenPLC value and update if necessary
    if gv_1.get_value() != modbus_client.read_signal(gv_1):
        gv_1.set_value(modbus_client.read_signal(gv_1))
        redis_client.set_value("gv_1", gv_1.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(gv_1)
