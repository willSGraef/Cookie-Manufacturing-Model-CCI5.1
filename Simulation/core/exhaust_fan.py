...
while not shutdown:
    ...
    exhaust_fan = redis_client.get("exhaust_fan")

    # Compare redis value to OpenPLC value and update if necessary
    if exhaust_fan.get_value() != modbus_client.read_signal(exhaust_fan):
        exhaust_fan.set_value(modbus_client.read_signal(exhaust_fan))
        redis_client.set_value("exhaust_fan", exhaust_fan.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(exhaust_fan)
