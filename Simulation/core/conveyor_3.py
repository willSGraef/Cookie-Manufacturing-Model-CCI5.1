...
while not shutdown:
    ...
    conveyor_3 = redis_client.get("conveyor_3")
    conveying_3 = redis_client.get("conveying_3")

    # Compare redis value to OpenPLC value and update if necessary
    if conveyor_3.get_value() != modbus_client.read_signal(conveyor_3):
        conveyor_3.set_value(modbus_client.read_signal(conveyor_3))
        redis_client.set_value("conveyor_3", conveyor_3.get_value())

    if conveying_3.get_value() != modbus_client.read_signal(conveying_3):
        conveying_3.set_value(modbus_client.read_signal(conveying_3))
        redis_client.set_value("conveying_3", conveying_3.get_value())

    # Write updated value back to OpenPLC
    modbus_client.write_signal(conveyor_3)
    modbus_client.write_signal(conveying_3)
